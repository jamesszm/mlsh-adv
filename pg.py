...
class PolicyGradient(object):
    def save_model_checkpoint(self, session, saver, filename, epoch_num):
        save_path = saver.save(session, os.path.expanduser(filename),
                               epoch_num)
    ...
    def sample_path(self, env, num_episodes=None):
        ...
        while num_episodes or t < self.config.batch_size:
            ...
            for step in range(self.config.max_ep_len):
                ...
                if config.examine_master and str(
                        config.env_name).startswith("Fourrooms"):
                    room = self.get_room_by_state(state)
                    rooms.append(room)

                    chosen_sub_policy, action = self.sess.run(
                        [self.chosen_index, self.sampled_action],
                        feed_dict={
                ...
                elif config.env_name == "Fourrooms-v1":
            ...
            if config.examine_master and str(config.env_name).startswith(
                    "Fourrooms") and self.config.render:
                print([(states[room], rooms[room])
